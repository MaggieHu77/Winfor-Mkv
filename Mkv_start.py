...
class Manage:
    ...
    def set_time_interval(self):
        ...
        start_t_input = input("-请输入回测开始时间，格式yyyy-mm【不需修改请直接回车】：\n").strip("\n\r")
        ...
        if not end_t_input:
            ...
            self.end_t = "-".join(end_t_input.split("-")[0:2])
    ...
    def get_backtest_t_seq(self):
        ...
        end_m = (datetime(*map(int, self.end_t.split("-")), 1) + relativedelta(months=1)).strftime(
            "%Y-%m")
    ...
    def run_optimizer(self):
        if self.mode == 1:
            ...
            print(f"-*-进入回测模式，检测到{len(self.t_seq)}个月回测时点-*-")
            self.set_params1()
        else:
            stamp = datetime.now()
    ...
    def write_output1(self):
        ...
        s1.title = "weights"
        s2 = wb.create_sheet()
        s2.title = "portfolio monthly returns"
        s3 = wb.create_sheet()
        ...
        for row in content1:
            s1.append(row)
        # 写第二张表回测权重持有至下一期的组合月收益率
        from dateutil.relativedelta import relativedelta
        from calendar import monthrange
        next_t = datetime(*map(int, self.t_seq[-1].split("-")), 1) + relativedelta(months=1)
        d = monthrange(next_t.year, next_t.month)[1]
        if not w.isconnected:
            w.start()
        next_t = datetime(next_t.year, next_t.month, d).strftime("%Y-%m-%d")
        returns = w.wsd(",".join(self.codes), "pct_chg", self.t_seq[1], next_t, "Period=M;showblank=0").Data
        returns.append([self.cash_r * 30] * len(returns[0]))
        returns = list(zip(*returns))
        from numpy import dot
        col22 = [dot(self.w_panel[self.t_seq[t]], returns[t]) for t in range(len(self.t_seq))]
        s2.append(self.t_seq[1:] + [next_t])
        s2.append(col22)
        # 写第三张表
        content3 = [col11, col12]
        for tt in self.t_seq:
            content3.append([tt] + self.mu_panel[tt])
        content3 = list(zip(*content3))
        for row in content3:
            s3.append(row)
        s3.append([self.cash_r] * len(self.t_seq))
        # 写回测期间协方差的表
